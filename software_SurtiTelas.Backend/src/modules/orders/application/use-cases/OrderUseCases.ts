...
export class UpdateOrderStatus {
  ...
  async execute(id: string, estado: OrderStatus, requestId?: string) {
      ...
      try {
        ...
        if (!existingReceipt) {
          const receipt = await this.repo.createReceipt({
            ...
            total: Number(updated.total),
            concepto: `Pedido ${updated.numero || updated.id} - ${updated.items} Ã­tems`,
    ...
    if (estado === 'Entregado') {
      ...
      // automÃ¡ticamente al entregar; cada pago debe ser registrado y
      // aprobado explÃ­citamente. El PaymentApprovedSubscriber crearÃ¡ la
...
export class UploadPaymentProof {
  constructor(private readonly repo: OrderRepository, private readonly eventBus?: EventBus) {}
  async execute(id: string, data: { url: string; nombreOriginal: string; mime: string; tamaÃ±o: number; cargadoPorId: string; estado: string; observaciones?: string }, requestId?: string) {
